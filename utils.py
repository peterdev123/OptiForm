...
def draw_dotted_hline(frame, y, start_x, end_x, line_color):
    for x in range(start_x, end_x + 1, 8):
        cv2.circle(frame, (x, y), 2, line_color, -1, lineType=cv2.LINE_AA)
    return frame
...
def get_landmark_features(kp_results, dict_features, feature, frame_width, frame_height):
    ...
    if feature == 'nose':
        ...
        return shldr_coord, hip_coord, knee_coord, ankle_coord, heel_coord, foot_coord
